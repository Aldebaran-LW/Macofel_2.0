...
async function main() {
  ...
  const productImages: Record<string, string> = {
    ...
    'Pincel para pintura 3 polegadas': 'https://cdn.abacus.ai/images/cd3e4364-39d8-40b5-8352-8a4f0553735c.png',
  ...
  for (const cat of categories) {
    const created = await mongoPrisma.category.create({ data: cat });
  ...
  for (const prod of products) {
    await mongoPrisma.product.create({ data: prod });
    console.log(`✅ Produto criado: ${prod.name}`);
  }

  console.log('\n🎉 Seed do MongoDB concluído!');
  console.log('📊 Resumo:');
  console.log(`   - ${await mongoPrisma.category.count()} categorias`);
  console.log(`   - ${await mongoPrisma.product.count()} produtos`);
}
