...
// Imagens de construção para rotação
...
export default function RotatingImages() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % constructionImages.length);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-full">
        {constructionImages.map((src, index) => {
          const isActive = index === currentIndex;
          const isNext = index === (currentIndex + 1) % constructionImages.length;
          const isPrev = index === (currentIndex - 1 + constructionImages.length) % constructionImages.length;
