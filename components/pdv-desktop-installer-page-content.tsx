...
type Audience = 'master' | 'admin' | 'painel';

const INTRO: Record<Audience, string> = {
  master:
    'Descarregue o programa de caixa (offline + sincronização com este site) para instalar no PC da loja. Apenas utilizadores Master devem distribuir este link.',
  admin:
    'Descarregue o programa de caixa (offline + sincronização com este site) para instalar nos PCs da loja. Utilizadores com acesso ao painel Admin podem obter o instalador aqui.',
  painel:
    'Descarregue o programa de caixa (offline + sincronização com este site) para instalar no PC do caixa. Em caso de dúvida, fale com a administração da loja.',
};
...
export function PdvDesktopInstallerPageContent({ backHref, backLabel, audience }: Props) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mb-8 text-gray-600">{INTRO[audience]}</p>
      ...
      <div className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-6 shadow-sm">
        ...
        {downloadUrl ? (
          <>
            ...
            <p className="mt-3 text-xs text-gray-600 break-all">Destino: {downloadUrl}</p>
          </>
        ) : (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
            ...
            <p className="mt-2 text-xs text-amber-800/80">
              Ver <code>public/downloads/README.md</code> no repositório Macofel e o README do projeto
              PDV-Macofel.
            </p>
          </div>
        )}
      ...
      <div className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          ...
          <strong>PDV-Macofel</strong> — <strong>não</strong> pela build do Next.js. Se o /loja
          parecer antigo, falta atualizar esse embed e fazer deploy.
        </p>
        <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-gray-600">
          <li>
            ...
            <code className="rounded bg-gray-100 px-1">PDV-Macofel</code>) ou defina a variável de
            ambiente <code className="rounded bg-gray-100 px-1">PDV_MACOFEL_ROOT</code> com o caminho
            absoluto.
          </li>
          <li>
            Na raiz do site Macofel: <code className="rounded bg-gray-100 px-1">npm run pdv:embed</code>{' '}
            (executa <code className="rounded bg-gray-100 px-1">build:embed</code> no PDV-Macofel e copia
            para <code className="rounded bg-gray-100 px-1">public/loja</code>).
          </li>
          <li>
            Commit de <code className="rounded bg-gray-100 px-1">public/loja/</code> (inclui{' '}
            <code className="rounded bg-gray-100 px-1">embed-version.txt</code>) e push. Na Vercel pode
            usar <code className="rounded bg-gray-100 px-1">PDV_WEB_EMBED_VERSION</code> para invalidar
            cache sem mexer em ficheiros.
          </li>
        </ol>
      </div>
      ...
      <div className="mt-8 space-y-4 text-sm text-gray-600">
        ...
        <ol className="list-decimal space-y-2 pl-5">
          ...
          <li>
            Se o Windows (SmartScreen) avisar “editor desconhecido”, usar “Mais informações” → “Executar
            mesmo assim”, se a origem for confiável.
          </li>
          <li>
            Copiar o ficheiro <code className="rounded bg-gray-100 px-1">.env</code> do PDV (com{' '}
            <code className="rounded bg-gray-100 px-1">MACOFEL_API_KEY</code> e URL do site) para a pasta
            de instalação ou configurar conforme o guia do PDV.
          </li>
